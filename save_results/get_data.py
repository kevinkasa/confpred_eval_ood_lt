...
def IN1k(exp_name: str, transform: torchvision.transforms, model):
    """
    Save inference results on imagenet dataset
...
def INv2(exp_name: str, transform, model):
    """
    Save inference results on imagenet-v2 dataset
...
def INa(exp_name: str, transform, model):
    """
    ...
    utils.save_results_IN200(save_path=savepath, model=model, data_loader=imagenet_a_loader, device=device,
                             exp_name=exp_name, indices=indices_in_1k)


def INr(exp_name: str, transform, model):
    """
    ...
    utils.save_results_IN200(save_path=savepath, model=model, data_loader=imagenet_r_loader, device=device,
                             exp_name=exp_name, indices=imagenet_r_mask)


def INc(exp_name: str, transform, model,
        corruptions=None):
    """
    Save inference results on imagenet-r dataset
    ...
    Args:
        exp_name: name of this exp / model
        corruptions: list of corruptions to test on
        transform: torchvision.transform for this dataset
        model: torchvision model used for inference
    """

    if corruptions is None:
        corruptions = ['contrast', 'brightness', 'motion_blur', 'gaussian_noise']
    print('Working on ImageNet-C: ')
    # ImageNet-C --Contrast corruption
    for corr in corruptions:
        print('Working on ImageNet-C: ' + corr)
        for i in range(1, 6):
...
def INw(exp_name: str, transform, model):
    """
    Save inference results on imagenet dataset
    ...
    print('Working on ImageNet-W')
    # Access the list of transformations in the existing test_transform object
    transform_list = transform.transforms

    # Insert the AddWatermark transformation before normalization
